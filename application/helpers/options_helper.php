...
/**
 * Generate a prepared query that will be queried with an array instead.
 *
 * @param array $SQL_ARRAY	An array populated with the information used to generate the query.
 * @param bool  $BINDINGS	BOOLEAN stating if we wish to use bindings with ? for queries instead.
 *
 * @return array|bool If the query is invalid return FALSE, else return ARRAY().
 */
function get_prepared_query($SQL_ARRAY = array(), $BINDINGS = FALSE)
{
	/**
	 * DEFAULT ARRAY USED FOR POPULATION
	 * ---------------------------------
		$MySQL = array(
		'SELECT'	=> array(),
		'FROM'		=> array(),
	...
	/**
	 * Sorry, we will not generate the query with an empty array!
	 */
	if (empty($SQL_ARRAY))
		return FALSE;

	/**
	 * Create empty arrays defined as $SQL and $VARIABLES to be populated. They will be used to
	 * create a valid SQL query for CI's $this->db->query() function.
	 */
	$SQL = array();
	$VARIABLES = array();

	/**
	 * Begin the creation of the SQL Query that will be returned as a result.
	 */
		...
		switch ($QUERY)
		{
			...
			case 'WHERE':
				...
				foreach ($STATEMENT as $FIELD => $VALUE)
				{
					...
					{
						if ($BINDINGS)
							array_push($_STATEMENT_CONDITIONS, "{$FIELD} = ?");
						else
					...
					else
					{
						...
						{
							if ($BINDINGS)
								array_push($_STATEMENT_CONDITIONS, "{$FIELD_OP[0]} " . str_replace('_', ' ', $FIELD_OP[1]) . " ?");
							else
								array_push($_STATEMENT_CONDITIONS, "{$FIELD_OP[0]} " . str_replace('_', ' ', $FIELD_OP[1]) . " {$VALUE}");
						}
	...
	if ($BINDINGS)
		return array('QUERY' => implode(' ', $SQL), 'VALUES' => $VARIABLES);
	else
		return array('QUERY' => implode(' ', $SQL));
}