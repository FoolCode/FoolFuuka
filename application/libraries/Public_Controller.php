...
class Public_Controller extends MY_Controller
{
	public function __construct()
	{
		...
		if($this->input->cookie('reply_password') == FALSE || strlen($this->input->cookie('reply_password')) < 3)
		{
			// create a new random password
			$this->load->helper('string');
			$rand_pass = random_string('alnum', 16);
			$this->input->set_cookie('reply_password', $rand_pass, 60*60*24*30);
			$this->fu->reply_password = $rand_pass;
		}
		else
		{
			$this->fu->reply_password = $this->input->cookie('reply_password');
		}

		$this->fu->reply_email = '';
		$this->fu->reply_name = '';

		if($this->input->cookie('reply_email') != FALSE)
		{
			$this->fu->reply_email = $this->input->cookie('reply_email');
		}

		if($this->input->cookie('reply_name') != FALSE)
		{
			$this->fu->reply_name = $this->input->cookie('reply_name');
		}
		...
		else
		{
			...
			{
				$selected_theme = $this->input->cookie('theme');
			}
		}
