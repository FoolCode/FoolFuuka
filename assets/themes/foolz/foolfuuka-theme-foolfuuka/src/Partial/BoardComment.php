                ...
                <div class="post_file">
                    ...
                    <?php if ($p_media->getMediaStatus($this->getRequest()) !== 'banned' || $this->getAuth()->hasAccess('media.see_hidden')) : ?>
                        ...
                        <a href="<?= $this->getUri()->create(((isset($modifiers['post_show_board_name']) && $modifiers['post_show_board_name']) ? '_' : $p->radix->shortname) . '/search/image/' . $p_media->getSafeMediaHash()) ?>" class="btnr parent"><?= _i('View Same') ?></a><a
                            href="http://google.com/searchbyimage?image_url=<?= trim($this->getUri()->create($p_media->getMediaLink($this->getRequest())),'/') ?>" target="_blank" class="btnr parent">Google</a><a
                            href="http://iqdb.org/?url=<?= trim($this->getUri()->create($p_media->getMediaLink($this->getRequest())),'/') ?>" target="_blank" class="btnr parent">iqdb</a><a
                            href="http://saucenao.com/search.php?url=<?= trim($this->getUri()->create($p_media->getMediaLink($this->getRequest())),'/') ?>" target="_blank" class="btnr parent">SauceNAO</a><?php if (!$p->radix->archive || $p->radix->getValue('archive_full_images')) : ?><a
