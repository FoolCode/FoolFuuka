var bindFunctions = function()
{
    ...
    jQuery("body").on("click",
        "a.[data-function], button.[data-function], input.[data-function], [data-function]",
        function(event) {
            var el = jQuery(this);
            ...
            switch (el.data("function")) {
				...
				case 'toggleDropdown':
					jQuery('.dropdown-menu-bla').toggle();
					break;
					
				case 'vote':
					jQuery.ajax({
						url: backend_vars.site_url + 'api/chan/vote',
						dataType: 'json',
                        type: 'POST',
                        cache: false,
                        data: {
							board: el.data('board'),
							doc_id: el.data('doc-id'),
							vote: el.data('vote'),
						...
						success: function(data) {
							...
							jQuery('.doc_id_' + el.data('doc-id') + ' .vote_number:eq(0)').text(count);
						}
					});
					break;
                ...
                case 'searchShow':
                    el.parent().find('.search-dropdown-menu').slideDown();
