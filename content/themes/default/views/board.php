...
<article id="<?= $op->num ?>" class="clearfix thread doc_id_<?= $op->doc_id ?> board_<?= $selected_radix->shortname ?>">
	...
	<header<?= (isset($op->report_status) && !is_null($op->report_status)) ? ' class="reported"' : '' ?>>
		<div class="post_data">
			...
			<a href="<?= site_url($selected_radix->shortname . '/thread/' . $op->num) . '#'  . $op->num ?>" data-post="<?= $op->num ?>" data-function="highlight">No.</a><a href="<?= site_url($selected_radix->shortname . '/thread/' . $op->num) . '#q' . $op->num ?>" data-post="<?= $op->num ?>" data-function="quote"><?= $op->num ?></a>
