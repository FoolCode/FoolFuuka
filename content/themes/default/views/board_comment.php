...
<article class="post doc_id_<?php echo $p->doc_id ?>
	...
	<?php if ($p->media) : ?> has_image clearfix<?php endif; ?>" id="<?php echo $p->num . (($p->subnum > 0) ? '_' . $p->subnum : '') ?>">

	<?php if ($p->preview_orig) : ?>
	<div class="post_file">
			<span class="post_file_controls">
			<?php if (!$selected_radix->hide_thumbnails || $this->tank_auth->is_allowed()) : ?>
				<a href="<?php echo site_url('@radix/' . $selected_radix->shortname . '/search/image/' . $p->safe_media_hash) ?>" class="btnr parent"><?php echo __('View Same') ?></a><a
					href="http://google.com/searchbyimage?image_url=<?php echo $p->thumb_link ?>" target="_blank" class="btnr parent">Google</a><a
					href="http://iqdb.org/?url=<?php echo $p->thumb_link ?>" target="_blank" class="btnr parent">iqdb</a><a
					href="http://saucenao.com/search.php?url=<?php echo $p->thumb_link ?>" target="_blank" class="btnr parent">SauceNAO</a>
				<?php endif; ?>
			</span>
	...
	<div class="thread_image_box">
		<a href="<?php echo ($p->media_link) ? $p->media_link : $p->remote_media_link ?>" target="_blank" rel="noreferrer" class="thread_image_link">
			...
			<img src="<?php echo site_url('content/themes/default/images/transparent_pixel.png') ?>" data-original="<?php echo $p->thumb_link ?>" <?php echo ($p->preview_w > 0 && $p->preview_h > 0) ? 'width="' . $p->preview_w . '" height="' . $p->preview_h . '" ' : '' ?>class="lazyload post_image<?php echo ($p->spoiler) ? ' is_spoiler_image' : '' ?>" data-md5="<?php echo $p->media_hash ?>" />
			<noscript>
				<a href="<?php echo ($p->media_link) ? $p->media_link : $p->remote_media_link ?>" target="_blank" rel="noreferrer" class="thread_image_link">
			...
			<?php else : ?>
			<img src="<?php echo $p->thumb_link ?>" <?php echo ($p->preview_w > 0 && $p->preview_h > 0) ? 'width="' . $p->preview_w . '" height="' . $p->preview_h . '" ' : '' ?>class="lazyload post_image<?php echo ($p->spoiler) ? ' is_spoiler_image' : '' ?>" data-md5="<?php echo $p->media_hash ?>" />
			<?php endif; ?>
		</a>
	</div>
	<?php endif; ?>
	...
	<header>
		<div class="post_data">
			...
			<span class="post_show_board">/<?php echo $selected_radix->shortname ?>/</span>
			<?php endif; ?>
			...
			<?php echo ($p->poster_hash_processed) ? '<span class="poster_hash">' . $p->poster_hash_processed . '</span>' : '' ?>
			<?php if ($p->capcode != 'N') : ?>
				<?php if ($p->capcode == 'M') : ?>
			...
			<?php if ($p->deleted == 1) : ?><span class="post_type"><img src="<?php echo site_url().'content/themes/'.($this->theme->get_selected_theme() ? $this->theme->get_selected_theme() : 'default').'/images/icons/file-delete-icon.png'; ?>" width="16" height="16" title="<?php echo form_prep(__('This post was deleted from 4chan manually.')) ?>"/></span><?php endif ?>
