	...
	<header>
		<div class="post_data">
			...
			<span class="post_controls">
				<?php if (isset($modifiers['post_show_view_button'])) : ?><a class="btnr" href="<?php echo site_url(array($this->fu_board, 'thread', $p->parent)) . '#' . $p->num . (($p->subnum) ? '_' . $p->subnum : '') ?>">View</a><?php endif; ?><a href="<?php echo site_url($this->fu_board . '/report/' . $p->doc_id) ?>" class="btnr parent" data-function="report" data-post="<?php echo $p->doc_id ?>" data-post-id="<?php echo $p->num . (($p->subnum > 0) ? ',' . $p->subnum : '') ?>" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true">Report</a><?php if ($p->subnum > 0 || $this->tank_auth->is_allowed()) : ?><a href="<?php echo site_url($this->fu_board . '/delete/' . $p->doc_id) ?>" class="btnr parent" data-function="delete" data-post="<?php echo $p->doc_id ?>" data-post-id="<?php echo $p->num . (($p->subnum > 0) ? ',' . $p->subnum : '') ?>" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true">Delete</a><?php endif; ?><?php if (false && $this->tank_auth->is_allowed()) : ?><a href="<?php echo site_url($this->fu_board . '/spam/' . $p->doc_id) ?>" class="btnr parent" data-function="spam" data-post="<?php echo $p->doc_id ?>" data-post-id="<?php echo $p->num . (($p->subnum > 0) ? '_' . $p->subnum : '') ?>" >Spam</a><?php endif; ?>
			</span>
