...
foreach ($posts as $key => $post) : ?>
	...
	<div id="p<?php echo $op->num ?>">
		...
		<span>
			File: <?php echo byte_format($op->media_size, 0) . ', ' . $op->media_w . 'x' . $op->media_h . ', ' . $op->media_filename; ?>
			<?php echo '<!-- ' . substr($op->media_hash, 0, -2) . '-->' ?>
		</span>
		[<a href="<?php echo site_url($this->fu_board . '/image/' . urlencode(substr($op->media_hash, 0, -2))) ?>">View Same</a>] [<a href="http://iqdb.org/?url=<?php echo $op->thumbnail_href ?>" target="_blank">iqdb</a>] [<a href="http://google.com/searchbyimage?image_url=<?php echo $op->thumbnail_href ?>" target="_blank">Google</a>] [<a href="http://saucenao.com/search.php?url=<?php echo $op->thumbnail_href ?>" target="_blank">SauceNAO</a>]
		<br>
		<a href="<?php echo ($op->image_href)?$op->image_href:$op->remote_image_href ?>" rel="noreferrer" target="_blank">
			<img class="thumb" src="<?php echo $op->thumbnail_href ?>" alt="<?php echo $op->num ?>" width="<?php echo $op->preview_w ?>" height="<?php echo $op->preview_h ?>" md5="<?php echo $op->media_hash ?>">
		</a>
		<?php endif; ?>

		<label>
			<span class="postername"><?php echo $op->name ?></span>
			<?php echo date('M/d/y(D)H:i', $op->timestamp) ?>
		</label>
