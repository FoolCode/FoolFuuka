			...
			<tr>
				...
				<td id="<?php echo ($p->subnum > 0) ? $p->num . '_' . $p->subnum : $p->num ?>" class="<?php echo ($p->subnum > 0) ? 'subreply' : 'reply' ?>">
					...
					<span id="norep<?php echo $p->num . '_' . $p->subnum ?>">
						<a href="<?php echo site_url($CI->fu_board . '/thread/' . $p->parent) . '#' . $p->num . '_' . $p->subnum ?>" class="quotejs">No.</a><a href="<?php echo ($thread_id == NULL) ? site_url($CI->fu_board . '/thread/' . $p->parent) . '#q' . $p->num . '_' . $p->subnum : 'javascript:quote(\'' . $p->num . ',' . $p->subnum . '\')'?>" class="quotejs"><?php echo $p->num . ',' . $p->subnum ?></a>
					</span>
					<?php else : ?>
					<span id="norep<?php echo $p->num ?>">
						<a href="<?php echo site_url($CI->fu_board . '/thread/' . $p->parent) . '#' . $p->num ?>" class="quotejs">No.</a><a href="<?php echo ($thread_id == NULL) ? site_url($CI->fu_board . '/thread/' . $p->parent) . '#q' . $p->num : 'javascript:quote(\'' . $p->num . '\')' ?>" class="quotejs"><?php echo $p->num ?></a>
					</span>
					<?php endif; ?>
