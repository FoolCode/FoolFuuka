	...
	public function toString()
	{
		...
		<article id="<?= $num ?>" class="clearfix thread doc_id_<?= $op->doc_id ?> board_<?= $op->radix->shortname ?>" data-doc-id="<?= $op->doc_id ?>" data-thread-num="<?= $op->thread_num ?>">
				...
				<time datetime="<?= gmdate(DATE_W3C, $op->timestamp) ?>" class="show_time" <?php if ($op->radix->archive) : ?> title="<?= __('4chan Time') . ': ' . gmdate('D M d H:i:s Y', $op->getOriginalTimestamp()) ?>"<?php endif; ?>><?= gmdate('D M d H:i:s Y', $op->timestamp) ?></time>
						...
						<a href="<?= \Uri::create([$op->radix->shortname, $op->_controller_method, $op->thread_num]) . '#'  . $num ?>" data-post="<?= $num ?>" data-function="highlight">No.</a><a href="<?= \Uri::create([$op->radix->shortname, $op->_controller_method, $op->thread_num]) . '#q' . $num ?>" data-post="<?= $num ?>" data-function="quote"><?= $num ?></a>
				...
				<a href="<?= \Uri::create([$op->radix->shortname, 'thread', $num]) ?>" class="btnr parent"><?= __('View') ?></a><a href="<?= \Uri::create([$op->radix->shortname, $op->_controller_method, $num]) . '#reply' ?>" class="btnr parent"><?= __('Reply') ?></a><?= (isset($post['omitted']) && $post['omitted'] > 50) ? '<a href="' . \Uri::create($op->radix->shortname . '/last/50/' . $num) . '" class="btnr parent">' . __('Last 50') . '</a>' : '' ?><?= ($op->radix->archive) ? '<a href="//boards.4chan.org/' . $op->radix->shortname . '/res/' . $num . '" class="btnr parent">' . __('Original') . '</a>' : '' ?><a href="#" class="btnr parent" data-post="<?= $op->doc_id ?>" data-post-id="<?= $num ?>" data-board="<?= htmlspecialchars($op->radix->shortname) ?>" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true" data-function="report"><?= __('Report') ?></a><?php if (\Auth::has_access('maccess.mod') || !$op->radix->archive) : ?><a href="#" class="btnr parent" data-post="<?= $op->doc_id ?>" data-post-id="<?= $num ?>" data-board="<?= htmlspecialchars($op->radix->shortname) ?>" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true" data-function="delete"><?= __('Delete') ?></a><?php endif; ?>
			</span>
						...
						<div class="btn-group post_mod_controls" style="clear:both; padding:5px 0 0 0;">
							...
							<button class="btn btn-mini" data-function="searchUser" data-board="<?= $op->radix->shortname ?>" data-board-url="<?= \Uri::create([$op->radix->shortname]) ?>" data-id="<?= $op->doc_id ?>" data-poster-ip="<?= Inet::dtop($op->poster_ip) ?>"><?= __('Search IP') ?></button>
							<?php if (Preferences::get('fu.sphinx.global')) : ?>
								<button class="btn btn-mini" data-function="searchUserGlobal" data-board="<?= $op->radix->shortname ?>" data-board-url="<?= \Uri::create([$op->radix->shortname]) ?>" data-id="<?= $op->doc_id ?>" data-poster-ip="<?= Inet::dtop($op->poster_ip) ?>"><?= __('Search IP Globally') ?></button>
								<?php endif; ?>
							<?php endif; ?>
						</div>
						<?php endif; ?>
		...
		<span class="omitted">
			<a style="display:inline-block" href="<?= \Uri::create([$op->radix->shortname, $op->_controller_method, $op->thread_num]) ?>" data-function="expandThread" data-thread-num="<?= $op->thread_num ?>"><i class="icon icon-resize-full"></i></a>
		...
		<article class="clearfix thread">
					<?php \Foolz\Plugin\Hook::forge('fu.themes.default_after_headless_open')->setParam('board', [isset($radix) ? $radix : null])->execute(); ?>
				<?php endif; ?>
