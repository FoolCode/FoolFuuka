...
<article id="<?= $num ?>" class="clearfix thread doc_id_<?= $op->doc_id ?> board_<?= $op->board->shortname ?>">
	...
	<header<?= (isset($op->report_status) && !is_null($op->report_status)) ? ' class="reported"' : '' ?>>
		<div class="post_data">
			...
			<?php if ($op->poster_country !== null) : ?><span class="post_type"><img title="<?= e($op->poster_country_name) ?>" src="<?= \Uri::base() ?>assets/famfamfam_flag_icons/png/<?= strtolower($op->poster_country) ?>.png" style="position:relative; bottom:1px" /></span><?php endif; ?>
